...
object DaoLitepal {
     ...
     * 获取搜索历史
     */
    fun getAllSearchInfo(title: String? = null): MutableList<HistoryTag> {
        return if (title == null) {
            LitePal.order("id desc").find(HistoryTag::class.java)
        } else {
            LitePal.where("title like ?", "%$title%").order("id desc").find(HistoryTag::class.java)
        }
    ...
    fun addSearchInfo(info: String): Boolean {
        ...
        val queryInfo = HistoryTag(id, info)
    ...
    fun deleteSearchInfo(info: String) {
        LitePal.deleteAllAsync(HistoryTag::class.java, "title = ? ", info).listen {
     ...
     */
    fun clearAllSearch() {
        LitePal.deleteAll(HistoryTag::class.java)
    }
    /* ***********************播放歌单操作***********************************/
